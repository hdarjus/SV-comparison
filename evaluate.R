...
stpar.c <- drop(extract(stfit.c, pars, inc_warmup = FALSE, permuted = FALSE)[,i.chain,])
stpar.nc <- drop(extract(stfit.nc, pars, inc_warmup = FALSE, permuted = FALSE)[,i.chain,])
...
as.data.frame(n.draws / rbind(
  ...
  Stan.c = sapply(c("phi" = "phi", "sigma" = "sigma", "mu" = "mu"), function (x) effectiveSize(drop(extract(stfit.c, x, inc_warmup = FALSE, permuted = FALSE)[,i.chain,]))),
  Stan.nc = sapply(c("phi" = "phi", "sigma" = "sigma", "mu" = "mu"), function (x) effectiveSize(drop(extract(stfit.nc, x, inc_warmup = FALSE, permuted = FALSE)[,i.chain,]))),
...
jalatent.c <- jafit.c[[i.chain]][, latent.names]
jalatent.nc <- jafit.nc[[i.chain]][, latent.names]
stlatent.c <- drop(extract(stfit.c, pars = latent.names, permuted = FALSE)[,i.chain,])
stlatent.nc <- drop(extract(stfit.nc, pars = latent.names, permuted = FALSE)[,i.chain,])

myvolplot(exp(as.matrix(jalatent.c)/2), dat)
myvolplot(exp(0.5 * apply(as.matrix(jalatent.nc), 2, function (x, mu, sigma) { sigma*x + mu }, japar.nc[, "mu"], japar.nc[, "sigma"])), dat)
myvolplot(exp(stlatent.c/2), dat)
myvolplot(exp(0.5 * apply(stlatent.nc, 2, function (x, mu, sigma) { sigma*x + mu }, stpar.nc[, "mu"], stpar.nc[, "sigma"])), dat)
